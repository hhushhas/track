...
void main() {
  ...
  float tone = smoothstep(0.08, 1.18, f);
  vec3 col = mix(baseColor, waveColor, tone);
...
vec3 dither(vec2 uv, vec3 color) {
  ...
  float threshold = bayerMatrix8x8[y * 8 + x] - 0.12;
...
function DitheredWaves({
  ...
  const uniformsRef = useRef({
    ...
    waveSpeed: new THREE.Uniform(0.035),
    waveFrequency: new THREE.Uniform(2.55),
    waveAmplitude: new THREE.Uniform(0.24),
    baseColor: new THREE.Uniform(new THREE.Color(...baseColor)),
    waveColor: new THREE.Uniform(new THREE.Color(...primaryColor)),
    mousePos: new THREE.Uniform(new THREE.Vector2(0, 0)),
    enableMouseInteraction: new THREE.Uniform(1),
    mouseRadius: new THREE.Uniform(0.85),
  })
  ...
  useEffect(() => {
    ...
    const resolution = uniformsRef.current.resolution.value
  ...
  useFrame(() => {
    ...
    if (!previousBaseColor.current.every((value, index) => value === baseColor[index])) {
      uniforms.baseColor.value.set(...baseColor)
      previousBaseColor.current = baseColor
    }
    if (!previousPrimaryColor.current.every((value, index) => value === primaryColor[index])) {
      uniforms.waveColor.value.set(...primaryColor)
  ...
  return (
    <>
      ...
      <EffectComposer>
        <RetroEffect colorNum={8} pixelSize={2} />
      </EffectComposer>
      <mesh
...
export function DitherBanner({
  ...
  const stableBaseColor = useMemo(() => baseColor, [baseColor])
  const stablePrimaryColor = useMemo(() => primaryColor, [primaryColor])
  ...
  return (
    <Canvas
      ...
      className="track-dither-banner"
      dpr={1}
      gl={{ antialias: true, preserveDrawingBuffer: true }}
    >
      <DitheredWaves baseColor={stableBaseColor} primaryColor={stablePrimaryColor} />
    </Canvas>
  )
}

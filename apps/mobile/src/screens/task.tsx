...
function errorMessage(failure: unknown) {
  return taskErrorMessage(failure);
}
...
export default function TaskScreen() {
  ...
  const navigationIdentity: MobileTaskIdentity | null = companyId && membershipId ? {
    archived: archive === '1',
  ...
  const markTaskNotificationsRead = useMutation(api.taskNotifications.markTaskRead);
  ...
  const markedTask = useRef<string | null>(null);

  useEffect(() => {
    if (!detail || markedTask.current === detail.task._id) return;

    markedTask.current = detail.task._id;
    void markTaskNotificationsRead({
      ...
      markedTask.current = null;
    });
  }, [companyId, detail, markTaskNotificationsRead, membershipId]);
      ...
      <View style={styles.screen}>
        <ScrollView
          ...
          {conflict ? (
            ...
            <TaskStateBanner action={{ label: 'Dismiss', onPress: () => setNotice('') }} icon="check-circle" message={notice} tone="success" />
          ) : null}
          ...
          <View style={[styles.hero, { backgroundColor: theme.backgroundElevated, borderColor: theme.hairline }]}>
            ...
            <View style={styles.destinationActions}>
              <TaskAction label="View Project" onPress={() => router.push(projectOverviewHref(project, navigationIdentity ? { ...navigationIdentity, archived: Boolean(navigationIdentity.archived) } : null) as Href)} />
              <TaskAction label="Show on board" onPress={() => router.push(taskListHref(project, navigationIdentity, undefined, undefined, {
          ...
          {tab === 'details' ? (
            <TaskDetailsTab
              ...
              onOpenSubtask={(item) => router.push(taskDetailHref(project, item.task.publicKey, navigationIdentity) as Href)}
      ...
      <OptionsSheet
        ...
        {field === 'status' ? (
          <SheetSection>
            ...
            {boards && (!board || board.states.length === 0) ? (
              <SheetNote>
        ...
        {field === 'description' ? (
          <>
            <SheetInput autoFocus label="Description" maxLength={4000} multiline onChangeText={setDescription} value={description} />
            <SheetRow
              disabled={busy}
              icon="check"
...
const styles = StyleSheet.create({
  destinationActions: { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.two },

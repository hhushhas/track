    ...
    <View style={styles.greetingIdentity}>
      ...
      <View style={styles.greetingCopy}>
        <View style={styles.contextLine}>
          <ThemedText numberOfLines={1} style={styles.eyebrow} themeColor="textSecondary" type="mono">{date}</ThemedText>
          <View style={[styles.contextDot, { backgroundColor: theme.hairline }]} />
          <ThemedText numberOfLines={1} style={styles.companyLabel} themeColor="accentStrong" type="mono">{companyLabel}</ThemedText>
        </View>
...
export function HomeSectionHeading({ icon, meta, title }: { icon: IconName; meta: string; title: string }) {
  const theme = useTheme();
  return <View accessibilityRole="header" style={styles.sectionHeading}><View style={styles.inline}><PlatformIcon color={theme.accentStrong} name={icon} size={17} /><ThemedText style={styles.sectionTitle} type="title">{title}</ThemedText></View><ThemedText themeColor="textTertiary" type="mono">{meta}</ThemedText></View>;
}
...
export function HomeAttentionCard({ item, onPress }: { item: MobileAttentionItem; onPress: () => void }) {
  ...
  return <Pressable accessibilityLabel={`${title}. ${attentionContext(item)}. ${action}`} accessibilityRole="button" android_ripple={{ color: theme.backgroundSelected }} onPress={onPress} style={({ pressed }) => [styles.attentionCard, { backgroundColor: pressed ? theme.backgroundElement : theme.backgroundElevated, borderColor: theme.hairline }]}> 
    <View style={styles.between}><View style={styles.cardContext}><View style={[styles.attentionIcon, { backgroundColor: urgent ? theme.dangerSoft : direct || suggestion ? theme.accentSoft : theme.backgroundElement }]}><PlatformIcon color={urgent ? theme.danger : direct || suggestion ? theme.accentStrong : theme.textSecondary} name={icon} size={15} /></View><View style={[styles.identifier, { backgroundColor: theme.backgroundSelected }]}><ThemedText numberOfLines={1} type="mono">{label}</ThemedText></View><ThemedText numberOfLines={1} style={styles.contextText} themeColor="textSecondary" type="caption">{attentionContext(item)}</ThemedText></View><ThemedText themeColor={urgent ? 'danger' : 'textTertiary'} type="caption">{relativeAttentionTime(item.createdAt)}</ThemedText></View>
    <ThemedText numberOfLines={2} style={styles.cardTitle} type={task ? 'title' : 'label'}>{title}</ThemedText>
    <View style={styles.cardFooter}><View style={[styles.cardAction, { backgroundColor: suggestion ? theme.text : 'transparent' }]}>{suggestion ? <PlatformIcon color={theme.background} name="check-circle" size={14} /> : null}<ThemedText style={{ color: suggestion ? theme.background : theme.accentStrong }} type="captionBold">{action}</ThemedText>{!suggestion ? <PlatformIcon color={theme.accentStrong} name="chevron-right" size={12} /> : null}</View>{!suggestion ? <ThemedText numberOfLines={1} style={styles.footerMeta} themeColor="textTertiary" type="caption">{attentionAction(item)}</ThemedText> : null}</View>
  </Pressable>;
}
...
export function HomeTaskCard({ assigneeName, item, onPress }: { assigneeName: string; item: HomeTask; onPress: () => void }) {
  ...
  return <Pressable accessibilityLabel={`${item.task.title}. ${item.task.publicKey}. ${[item.companyName, item.project.name].filter(Boolean).join(', ')}`} accessibilityRole="button" android_ripple={{ color: theme.backgroundSelected }} onPress={onPress} style={({ pressed }) => [styles.taskCard, { backgroundColor: pressed ? theme.backgroundElement : theme.backgroundElevated, borderColor: theme.hairline }]}> 
    <View style={styles.between}><View style={styles.taskBadges}><Badge text={item.task.publicKey} /><Badge text={item.state?.name ?? 'Open'} />{item.task.priority !== 'none' ? <Badge danger={urgent} text={priority} /> : null}</View>{due ? <ThemedText themeColor={urgent ? 'danger' : 'textSecondary'} type="caption">{due}</ThemedText> : null}</View>
...
function Badge({ danger, text }: { danger?: boolean; text: string }) { const theme = useTheme(); return <View style={[styles.statusBadge, { backgroundColor: danger ? theme.dangerSoft : theme.backgroundSelected }]}><ThemedText adjustsFontSizeToFit minimumFontScale={0.8} numberOfLines={1} themeColor={danger ? 'danger' : 'textSecondary'} type="mono">{text}</ThemedText></View>; }
...
const styles = StyleSheet.create({
  ...
  attentionIcon: { alignItems: 'center', borderRadius: Radius.small, height: 28, justifyContent: 'center', width: 28 },
  ...
  cardFooter: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', paddingTop: Spacing.two }, cardTitle: { lineHeight: 20 },
  ...
  divider: { height: StyleSheet.hairlineWidth }, eyebrow: { fontSize: 10.5, letterSpacing: 0.55, lineHeight: 14 }, filterCount: { fontVariant: ['tabular-nums'] }, filterRail: { borderCurve: 'continuous', borderRadius: Radius.medium, overflow: 'hidden' }, filters: { gap: 2, padding: Spacing.one }, footerMeta: { flex: 1, marginLeft: Spacing.two, textAlign: 'right' },
  ...
  identifier: { borderRadius: 2, flexShrink: 1, paddingHorizontal: 6, paddingVertical: 2 }, inline: { alignItems: 'center', flexDirection: 'row', gap: Spacing.one },
  ...
  scope: { alignItems: 'center', flexDirection: 'row', gap: Spacing.one, maxWidth: 112 }, sectionHeading: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', minHeight: 22 }, sectionTitle: { fontSize: 16, lineHeight: 22 }, statusBadge: { borderRadius: Radius.small, paddingHorizontal: Spacing.one, paddingVertical: 2 },
  taskBadges: { alignItems: 'center', flex: 1, flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.one, paddingRight: Spacing.two }, taskCard: { borderCurve: 'continuous', borderRadius: Radius.large, borderWidth: StyleSheet.hairlineWidth, boxShadow: '0 2px 8px rgba(0,0,0,0.045)', gap: Spacing.three, minHeight: 126, overflow: 'hidden', padding: Spacing.four }, taskContext: { alignItems: 'center', flex: 1, flexDirection: 'row', gap: Spacing.one, minWidth: 0 }, taskTitle: { fontSize: 16, lineHeight: 22 },
});
